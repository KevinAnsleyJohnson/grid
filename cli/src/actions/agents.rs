...
pub fn do_list_agents(url: &str, service_id: Option<String>, format: &str, line_per_role: bool) -> Result<(), CliError> {
...
fn print_csv(column_names: Vec<&str>, row_values: Vec<Vec<String>>) {
    ...
    for i in 0..column_names.len() {
        header_row += &format!("\"{}\",", column_names[i]);
    }
    header_row.pop();
    println!("{}", header_row);
